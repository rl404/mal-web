...
const Top = (props) => {
  ...
  const refresh = async () => {
    const result = await reparse(cons.ANIME_TYPE, state.data.id);
              ...
              <Grid container spacing={1} className={classes.content}>
                ...
                <Grid item xs={12}>
                  {state.data.genres.map(genre => {
                    return (
                      <Link to={`/search/anime?genre=${genre.id}`} key={genre.id}>
              ...
              <Grid container spacing={1}>
                <Grid item xs>
                  <Stats
                    ...
                    name='Rank'
                    onClick={() => props.showHistoryModal(cons.ANIME_TYPE, state.data.id)}
                  />
                </Grid>
                <Grid item xs>
                  <Stats
