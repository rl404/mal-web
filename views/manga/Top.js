...
const Top = (props) => {
  ...
  const refresh = async () => {
    const result = await reparse(cons.MANGA_TYPE, data.id)
    ...
    <Grid container spacing={1}>
      ...
      <Grid item xs={12} md={9}>
        <Grid container spacing={1} className={classes.content}>
          ...
          {props.mangalist && props.mangalist[data.id] ?
            <Grid item>
              <Tooltip placement='bottom' title={cons.MANGA_USER_STATUS[props.mangalist[data.id].status]}>
                <span className={classes.user} style={{ backgroundColor: statusColor[props.mangalist[data.id].status] }} />
              </Tooltip>
            </Grid>
            : null}
          <Grid item>
          ...
          <Grid item>
            <a href={`${cons.MAL_URL}/${cons.MANGA_TYPE}/${data.id}`} target='_blank' rel='noopener noreferrer'>
        ...
        <Grid container spacing={1} className={classes.content}>
          ...
          <Grid item xs={12}>
            {data.genres.map(genre => {
              return (
                <Link href={`/search/manga?genre=${genre.id}`} key={genre.id}>
                  <a>
        ...
        <Grid container spacing={1}>
          <Grid item xs>
            <Stats
              ...
              name='Rank'
              onClick={() => props.showHistoryModal(cons.MANGA_TYPE, data.id)}
            />
          </Grid>
          <Grid item xs>
            <Stats
